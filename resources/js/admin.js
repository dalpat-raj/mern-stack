...
function initAdmin(socket){
    ...
    function renderItems(items){
        ...
        return parsedItems.map((menuItem) => {
            return `
            <p>${ menuItem.title} - ${ menuItem.qty } pcs </p>
            `
        }).join('')
    }
    ...
    function renderPrice(items){
        let parsedItems = Object.values(items)
        return parsedItems.map((menuItem) => {
            return `<p>₹ ${ menuItem.price * menuItem.qty}</p>`
        }).join('')
    }
    ...
    function generateMarkup(orders) {
        return orders.map(order => {
            ...
            <td class="border px-4 py-2">${ order.customerId.firstName }</td>
            <td class="border px-4 py-2">${ order.customerId.phone }</td>
            <td class="border px-4 py-2">${ order.area }<br> ${order.city}</td>
